...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    @Query(value = "SELECT * FROM bookings WHERE booker_id = ?1 AND start_time < NOW() AND end_time > NOW()", nativeQuery = true)
    List<Booking> findAllByBookerIdAndCurrent(int bookerId);

    @Query(value = "SELECT * FROM bookings WHERE booker_id = ?1 AND end_time < NOW()", nativeQuery = true)
    List<Booking> findAllByBookerIdAndPast(int bookerId);

    @Query(value = "SELECT * FROM bookings WHERE booker_id = ?1 AND start_time > NOW()", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM bookings WHERE item_id IN (SELECT id FROM items WHERE owner_id = ?1) AND start_time < NOW() AND end_time > NOW()", nativeQuery = true)
    List<Booking> findAllByOwnerIdAndCurrent(int ownerId);

    @Query(value = "SELECT * FROM bookings WHERE item_id IN (SELECT id FROM items WHERE owner_id = ?1) AND end_time < NOW()", nativeQuery = true)
    List<Booking> findAllByOwnerIdAndPast(int ownerId);

    @Query(value = "SELECT * FROM bookings WHERE item_id IN (SELECT id FROM items WHERE owner_id = ?1) AND start_time > NOW()", nativeQuery = true)
